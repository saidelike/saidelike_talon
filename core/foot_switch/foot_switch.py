...
# It is very reliable and fast to do it on the "down" events on Windows
...
ctx_vlc_control_linux = Context()
ctx_vlc_control_linux.matches = r"""
tag: user.vlc_control
os: linux
"""

# On Windows, we can use the "down" events but on Linux it fails
...
# NOTE: this is less reliable than Windows and sometimes we need to push the pedal several times
@ctx_vlc_control_linux.action_class("user")
class VlcControlLinuxActions:
    def foot_switch_top_down():
        # actions.user.debug(f"VlcControlLinuxActions:foot_switch_top_down()")
        pass  # disable scrolling feature

    def foot_switch_top_up(held: bool):
        # actions.user.debug(f"VlcControlLinuxActions:foot_switch_top_up()")
        pass

    def foot_switch_center_down():
        # actions.user.debug(f"VlcControlLinuxActions:foot_switch_center_down()")
        pass

    def foot_switch_center_up(held: bool):
        # actions.user.debug(f"VlcControlLinuxActions:foot_switch_center_up()")
        # global VLC shortcut to play/pause VLC
        actions.key("shift-ctrl-f12")

    def foot_switch_left_down():
        # actions.user.debug(f"VlcControlLinuxActions:foot_switch_left_down()")
        pass  # disable going back feature

    def foot_switch_left_up(held: bool):
        # actions.user.debug(f"VlcControlLinuxActions:foot_switch_left_up()")
        # global VLC shortcut to navigate left
        actions.key("shift-ctrl-f10")
        pass

    def foot_switch_right_down():
        # actions.user.debug(f"VlcControlLinuxActions:foot_switch_right_down()")
        pass

    def foot_switch_right_up(held: bool):
        # actions.user.debug(f"VlcControlLinuxActions:foot_switch_right_up()")
        # global VLC shortcut to navigate right
        actions.key("shift-ctrl-f11")
